...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Folder containing barcode*.paf files (default: results/mapping_counts)",
    )
    parser.add_argument(
        "-o",
        "--output_folder",
    ...
    parser.add_argument(
        ...
        help="Two column tab-delimited file with the accession and genus information (default: data/genome_to_genus.tsv)",
    )
    parser.add_argument(
        "-s",
        "--stats_file",
...
def main():
    ...
    otu_table_multi_tax = taxon_df.merge(
        otu_table_multi, on="tname", how="right"
    ).set_index("tname")
    ...
    if stats_path and stats_path.exists():
        ...
        # Derive sample names from the stats file, more flexibly
        stats["sample"] = stats["file"].apply(lambda x: Path(x).stem)

        # Create sample name lists from both sources
        paf_samples = counts["sample"].unique().tolist()
        stats_samples = stats["sample"].unique().tolist()

        # Try to match samples even if names differ slightly
        matched_samples = {}
        for paf_s in paf_samples:
            for stats_s in stats_samples:
                # A simple heuristic: if paf_s is contained in stats_s or vice versa
                if paf_s in stats_s or stats_s in paf_s:
                    matched_samples[stats_s] = paf_s
                    break

        # Apply mapping if matches found
        if matched_samples:
            stats["sample"] = stats["sample"].replace(matched_samples)
        else:
            print(
    ...
    # Generate OTU table
    otu_table = (
        counts.pivot(index="tname", columns="sample", values="reads")
        .fillna(0)
        .astype(int)
    )
    ...
    counts_genus = counts_tax.groupby(["sample", "genus"], as_index=False)[
        "reads"
    ].sum()

    # Generate OTU table
    otu_table_genus = (
        counts_genus.pivot(index="genus", columns="sample", values="reads")
        .fillna(0)
        .astype(int)
    )
