...
After we better understand the quality of our amplicon long-read data we can then:
...
- Investigate the Nanoplot LengthvsQualityScatterPlot that you generated in the last exercise and decide on the right thresholds to use with chopper to:
...
-  NanoPlot
-  seqkit
   -  **Important**: Be sure that you add the barcodeID to the output file, we need this later!
...
- Input: 3785 reads
- Passed filtering: 2976 reads (~80%). You ideally want to keep 50%-90% of the reads for the down-stream analyses depending on the quality of the data and how many reads you generated.
...
To classify our filtered Nanopore reads, we use **Minimap2** to align each read against a reference database containing the 16S rRNA sequences from all the strains used in our lab experiments. In other words, minimap compares how "similar" each read is compared to every reference sequence. One or more alignments may be recorded depending on how well the read matches the references. 

Alignment-based tools like Minimap2 are well-suited for Nanopore data because they tolerate mismatches and small insertions or deletions, which are common in error-prone long reads. The output of Minimap2 is a **PAF** (Pairwise mApping Format) file, which contains information about how well each read aligns to the reference sequences.
...
Next, you will align your filtered reads against the reference database.
...
1. There are 3076 alignments for 2976 reads. There might be different reasons for seeing more reads aligning than expected 
   1. If your experiment included Pseudomonas strains you likely will have more alignments than reads. The higher number of alignments occurs because some reads might align equally well to multiple reference 16S sequences, these are multi-mapping reads.
...
Instead of using a single pre-built program, we will use a custom Python script to perform this step. So why Python o R? 
