...
Open your terminal is open, and get yourself oriented by typing your first command and then pressing enter: 
...
Throughout this tutorial, you’ll notice that we will use `ls` frequently. There’s a reason for that:

When working with data, sanity checks are essential, because it is easy to make mistakes, overwrite files, or lose track of where things are. Using simple commands like `ls`, `pwd, `wc`, or `grep` helps you verify what’s happening to your files at every step of an analysis workflow.
...
At some point, you’ll want to know what options a command has or how it works. In this case, you first should check if **manual pages** (or *man pages*) are available for the command by typing `man` followed by the command name:
...
Next, let’s download a fasta file that contains the sequence of a 16S rRNA gene. This will file will be useful to learn about some other commands. Below, we use the `wget` command to fetch a fasta file from an online website as follows:
...
3. What does it mean if one genome has more 16S rRNA gene sequences than another? 
...
- `wget github_path/${accession}.fasta.gz -P playground` downloads the compressed fasta file for each accession into the playground folder. Here, the `${accession}` will get replaced with one line in `accessions.txt`
...
1. Combine the 16S rRNA gene sequences from human-associated strains (GCF Prefix) into a new file that should be stored in the results folder
